    ...
    public ResponseEntity<?> updateItem(@PathVariable Long id, @RequestBody Item updatedItem) {
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getId().equals(id)) {
                ...
                existing.setName(updatedItem.getName() != null ? updatedItem.getName() : existing.getName());
                existing.setCategory(updatedItem.getCategory() != null ? updatedItem.getCategory() : existing.getCategory());
